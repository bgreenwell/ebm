...
\title{Explainable Boosting Machines in R with the ebm Package}
...
predicted outcome (a larger outcome is better here). This confirms that
...
negative and non-negative values of \texttt{last.slope.weight} (the
...
g\left(E\left[Y|\boldsymbol{x}\right]\right) = \beta_0 + \sum_{i=1}^p \beta_if_i\left(x_i\right),
  \label{eqn:ebm}
\end{equation} where the term weights/coefficients,
\(\left\{\beta_i\right\}_{i=0}^p\), are estimated from the LASSO.
Ideally, many of the term weights will be zero, meaning those terms can
be dropped from the model. Oftentimes, as will be shown here for the ALS
example, this can result in a substantial reduction of the number of
terms in the final EBM model.
...
pairwise interactions, and an intercept. The LASSO model will
essentially treat each individual term as a feature in the regression
model. To that end, we'll next construct data matrices consisting of the
...
\(f_j\left(x_{ij}\right)\), over the training set (not counting the
intercept). These data matrices will be used as input to the LASSO in
the next step.
