...
by replacing the systematic component of \eqref{eqn:glm} with a sum of
nonlinear smooth functions to produce \begin{equation}
...
\section{Explainable boosting machines}\label{sec:ebm}

In Section \ref{sec:ebm}. Explainable boosting machines
\citep{nori-2019-interpret}, or EBMs for short, are a modern class of
GA\textsuperscript{2}Ms, that can offer both competitive accuracy and
explicit transparency through exact explainability, which are often
considered to be two opposing goals of a machine learning (ML) model.
For example, full-complexity models, like random forests
\citep{breiman-2001-rf}, tend to be highly competitive in terms of
accuracy, but are readily less transparent and explainable (due in part
to the high-order interaction effects often captured by such black-box
models). Essentially, with EBMs, it's possible to ``have your cake and
eat it too.''
...
where, similar to GA\textsuperscript{2}Ms \eqref{eqn:ga2m},
...
Similar to GA\textsuperscript{2}Ms, the pairwise interaction terms are
determined automatically using the FAST algorithm described in
\citet{lou-2013-ga2m}. The primary difference between EBMs and
GA\textsuperscript{2}Ms is in how the shape functions are estimated. In
particular, EBMs use \emph{cyclic gradient boosting}
\citep{nori-2019-interpret, wick-2019-cyclic} to estimate the shape
functions for each feature (and selected pairs of interactions) in a
round-robin fashion using a low learning rate to help ensure that the
order in which the feature effects are estimated does not matter.
Apparently, estimating each feature one-at-a-time in a round-robin
fashion also helps mitigate potential issues with
...
production. To this end, \citet{greenwell-2023-explainable} proposed a
simple solution, which is illustrated via an example in a later section.
...
the R interface does not currently support regression settings,
monotonic constraints, or local explanations. Further, the plotting
functionality of the \CRANpkg{interpret} R package \citep{R-interpret}
...
that cannot be replicated through use of the official \pkg{interpret}
package in R.
...
\item
  ...
  Python module (e.g., Poisson deviance for modeling counts).
\item
...
to interface with the underlying Python module \pkg{interpret}.
...
Personally, I prefer to use pip and pyenv to manage my Python
environments, but virtual environments and conda environments are safer
and considered best practices. Therefore, in this section, I'll briefly
cover setting up a simple virtual environment with the necessary
dependencies. For further details and other available options, see
\pkg{reticulate}'s extensive documentation. In particular, read more
about configuring Python, setting up virtual environments, and
installing Python libraries in the the following vignettes:
...
A Python virtual environment is just an isolated space with a Python
interpreter and installed modules that are independent of your
system-installed Python environment. Virtual environments effectively
allow you to manage Python versions and dependencies separately across
different projects. Here, we'll create a new virtual environment called
\texttt{ebm-rjournal} and install the required \pkg{interpret} module.
Note that it is not necesary to do this through \pkg{reticulate}. As
noted above, I prefer to do this through the command line with \pkg{pip}
\citep{Py-pip} and \pkg{pyenv} \citep{Py-pyenv}. Furthermore, tools for
creating and managing virtual environments are part of the standard
Python library (as well as external packages), so it's possible to set
up virtual environments manually if you know what you're doing.
...
called \texttt{r-ebm} by default (see \texttt{?ebm::install\_interpret}
for details)!
...
the cumulative gains computed below:
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/tic-ebm-calibration-1} 
...
module or even exporting the fitted model to ONNX (Open Neural Network
Exchange) via the \pkg{ebm2onnx} module, depending on the need. For more
details on persisting a fitted model in Python visit
...
convenience, we'll also use \CRANpkg{patchwork} \citep{R-patchwork} for
configuring the layout for displays with multiple plots.
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/tic-ebm-importance-1} 
...
practice to include some information regarding the distribution of the
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/tic-ebm-term-PPERSAUT-1} 
...
{\centering \includegraphics[width=1\linewidth]{images/screenshot} 
...
decreasing. For instance, a monotonic increasing relationship would
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/tic-ebm-term-APERSAUT-1} 
...
Explanations) \citep{ribeiro-2016-lime}. Techniques like these typically
make assumptions about independence and so forth and only provide an
approximation to how features contribute to a model's output. With EBMs,
we can visualize exactly how each term in the model contributes to a
specific prediction (this is true for GAMs in general).
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/tic-ebm-local-1} 
...
\citet[p. 349]{efron-2016-casi}. A description of the data, along with
the original source and download instructions, can be found at
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/als-ebm-importance-1} 
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/als-ebm-pairwise-interaction-1} 
...
time. We propose a simple solution based on the least absolute shrinkage
and selection operator (LASSO) \citep{tibshirani-1996-lasso} that can
help introduce sparsity by reweighting the individual model terms and
removing the less relevant ones, thereby allowing these models to
maintain their transparency and relatively fast scoring times in
higher-dimensional settings. In short, post-processing a fitted EBM with
many (i.e., possibly hundreds or thousands) of terms using the LASSO can
help reduce the model's complexity and drastically improve scoring time.
For methodological (and software) details, see
\citet{greenwell-2023-explainable}.

We'll illustrate the basic idea using the previous model, which contains
1 total terms comprised of the main effects, pairwise interactions, and
an intercept.
...
Next, we use the \CRANpkg{glmnet}
\citep{friedman-2010-regularization, tay-2023-elastic} package to fit
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/als-ebm-lassso-plot-1} 

}

\caption[ABC]{ABC.}\label{fig:als-ebm-lassso-plot}
\end{figure}
\end{Schunk}

The test MSE for the compressed model is 0.262, which is actually better
than the original model with all 702 terms! Quite impressive in this
case.

Lastly, we can call the \texttt{\$scale()} and \texttt{\$sweep()}
methods to reweight and purge any unused terms from the compressed EBM
model:
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/als-ebm-lasso-scale-1} 

}

\caption[ABC]{ABC.}\label{fig:als-ebm-lasso-scale}
\end{figure}
\end{Schunk}

And just as a sanity check, we can see that the compressed (i.e.,
reduced) model does produce the right predictions by comparing it to the
output from the corresponding LASSO model:
...
the \pkg{interpret} Python library using \pkg{reticulate}.

\subsection{Parallelism in EBMs}\label{parallelism-in-ebms}

The core implementation of EBMs through the \pkg{interpret} Python
library is parallelizable, and takes advantage of \pkg{joblib}
\citep{Py-joblib} to provide multi-core and multi-machine
parallelization. Note that this differs from traditional parallelization
in R through packages like \pkg{parallel} \citep{R-parallel}. Here the
parallelization is taken care of completely on the Python side.
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/ebm-parallelism-1} 

}

\caption[ABC]{ABC. Left: ABC. Right: ABC.}\label{fig:ebm-parallelism}
\end{figure}
\end{Schunk}

\subsection{Merging EBMs into one}\label{merging-ebms-into-one}

The underlying \pkg{interpret} Python library allows you to merges EBM
models trained on similar data sets that have the same set of features.
This can be useful for a number of reasons. For instance, you can fit
several independent EBM models to implement your own outer bagging
strategy. Or perhaps you've fit several EBMs over time as data have
become available and you want to merge several of the models into one.
...
fitted below, but the merged model displays uncertaintly in the learned
graph for \texttt{cyl} (i.e., the number of cylinders).
...
{\centering \includegraphics[width=1\linewidth]{greenwell-ebm_files/figure-latex/ebm-merging-1} 

}

\caption[ABC]{ABC.}\label{fig:ebm-merging}
\end{figure}
\end{Schunk}
...
this paper. A reasonable strategy is to train an initial model using the
...
can be used to graphically examine the model at both the global and
local level. For instance, a model can be explained on the global level
by visualizing term importance scores, main effects, and important
pairwise interactions.

In this paper, we showed how to fit EBMs using the \pkg{ebm} package,
which provides a direct interface to the \pkg{interpret} Python library.
...
\pkg{interpret} package in R. For instance, I showed how to fit both
classification and regression models (including a model for counts using
Poisson deviance). I showed how to explain an EBM's output at both the
global and local level using either static or interactive plots. Further
code examples also demonstrated how to use some of the more advanced
functionality, like parallelism, merging several EBM models into one,
and model compressing (i.e., feature/term reduction) by post-processing
an EBM model with the LASSO. Further improvements to the \pkg{ebm}
package can also be made by providing explicit support for converting
fitted EBM models to ONNX \citep{onnx} using the wonderful
\pkg{ebm2onnx} package \citep{Py-ebm2onnx}, and model editing via GAM
changer \citep{Py-gamchanger}.
